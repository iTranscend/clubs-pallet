...
fn adds_a_new_member_to_a_club() {
    ...
    let member: <Test as frame_system::Config>::AccountId = account("", 1, SEED);
		...
		assert_ok!(TemplateModule::add_member(Origin::root(), club.clone(), member.clone()));
...
fn remove_a_member_from_a_club() {
    ...
    // assert the last event
		...
		// now remove member
		assert_ok!(TemplateModule::remove_member(Origin::root(), club.clone(), member.clone()));
...
fn attempts_to_add_adds_an_already_existing_member_to_a_club() {
    ...
    assert_noop!(
			TemplateModule::add_member(Origin::root(), club.clone(), member_2.clone()),
...
fn attempts_to_add_a_member_to_a_non_existent_club() {
    ...
    // Ensure error is thrown
    assert_noop!(
			TemplateModule::add_member(Origin::root(), club.clone(), member.clone()),
...
fn attempts_to_remove_a_non_existent_member_from_a_club() {
    ...
    // Ensure error is thrown
    assert_noop!(
			TemplateModule::remove_member(Origin::root(), club.clone(), member.clone()),
